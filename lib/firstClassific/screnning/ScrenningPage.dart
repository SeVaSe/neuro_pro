...
class ScrenningPage extends StatefulWidget {
  final TempStorage tempStorage; // Хранилище данных

  const ScrenningPage({Key? key, required this.tempStorage}) : super(key: key);

  @override
  _ScrenningPageState createState() => _ScrenningPageState();
}

class _ScrenningPageState extends State<ScrenningPage> {
...
// Позиция пальца для лупы
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      onWillPop: () async {
        final exit = await showExitConfirmationTempReymDialog3(context, widget.tempStorage);
        return exit ?? false;
      },
      child: Scaffold(
                            ...
                            onPressed: () async {
                              ...
                              if (exit ?? false) {
                                Navigator.pushAndRemoveUntil(
                                  context,
                                  MaterialPageRoute(builder: (context) => HomeScreen(tempStorage: widget.tempStorage)),
                                      (route) => false,
                                );
                              }
                            },
